...
export default function ShoppingListOverView() {
	...
	const renderRow = (item: IShoppingList, idx: number) => {
		const pallete = paletteFor(idx, isDark);
		const count = item.itemsProduct?.length ?? 0;
		return (
			<Pressable
				key={item._id}
				style={[styles.row, { backgroundColor: Colors.surfaceBackgroundColor, borderColor: Colors.borderColor }]}
				onPress={() => item._id && router.push(`/list/${item._id}`)}
				onLongPress={() => handleDelete(item)}
			>
				<View style={[styles.rowIcon, { backgroundColor: pallete.bg }]}>
					<MaterialIcons name={pallete.icon as never} size={22} color={pallete.iconColor} />
				</View>
				<View style={styles.rowBody}>
					<Text style={[styles.rowName, { color: Colors.primaryTextColor }]}>{item.name}</Text>
					<Text style={[styles.rowSub, { color: Colors.secondaryTextColor }]}>
	...
	return (
		<View style={[styles.root, { backgroundColor: Colors.screenBackgroundColor }]}>
			...
			<View style={[styles.header]}>
				<Text style={[styles.headerTitle, { color: Colors.primaryTextColor }]}>Lista de Compras</Text>
			</View>

			<ScrollView
				contentContainerStyle={[styles.scroll, { paddingBottom: TAB_TOTAL + 80 }]}
			...
			>
				...
				{/* Recently created */}
				<Text style={[styles.sectionLabel, { color: Colors.sectionLabelTextColor, marginTop: 20 }]}>
					CREADO RECIENTEMENTE
				</Text>

				{loading ? (
					<View style={[styles.row, { backgroundColor: Colors.surfaceBackgroundColor, borderColor: Colors.borderColor, justifyContent: 'center' }]}>
						<Text style={[styles.rowSub, { color: Colors.secondaryTextColor }]}>Cargando...</Text>
					</View>
				) : filteredLists.length === 0 ? (
					<View style={styles.emptyWrap}>
						<View style={[styles.emptyIcon, { backgroundColor: `${PRIMARY}18` }]}>
							<MaterialIcons name="add-shopping-cart" size={36} color={PRIMARY} />
						</View>
						<Text style={[styles.emptyTitle, { color: Colors.primaryTextColor }]}>Sin listas aún</Text>
						<Text style={[styles.emptySub, { color: Colors.secondaryTextColor }]}>
			...
			>
				<View style={styles.sheetContent}>
					<View style={styles.sheetHeader}>
						<Text style={[styles.sheetTitle, { color: Colors.primaryTextColor }]}>Nueva Lista</Text>
						<Pressable
							style={[styles.sheetClose, { backgroundColor: Colors.inputBackgroundColor }]}
					...
					<Text style={[styles.inputLabel, { color: Colors.secondaryTextColor }]}>NOMBRE DE LA LISTA</Text>
