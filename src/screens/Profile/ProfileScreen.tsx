...
const ProfileScreen = () => {
	...
	const handleImageSelection = async (mode: "camera" | "gallery") => {
		try {
			...
			if (!result.canceled && result.assets && result.assets.length > 0) {
				...
				Alert.alert(
					"Guardar Imagen",
					"¿Realmente deseas guardar esta imagen como foto de perfil?",
					[
						{ text: "Cancelar", style: "cancel" },
						{ text: "Guardar", onPress: () => uploadAvatar(selectedUri) },
					],
				);
			}
		} catch (error) {
	...
	const uploadAvatar = async (uri: string) => {
		...
		setUploading(true);
		try {
			const uploadRes = await mediaService.uploadImage(uri);
	...
	return (
		<View style={{ flex: 1, backgroundColor }}>
			...
			<View
				style={{
					height: Platform.OS === "ios" ? 54 : (StatusBar.currentHeight ?? 28),
				}}
			/>
				...
				<View style={styles.section}>
					...
					<View style={[styles.card, { backgroundColor: cardBg, borderColor }]}>
						...
						>
							...
							{theme === "system" && (
								<MaterialIcons name="check" size={20} color="#FF803E" />
							)}
						</TouchableOpacity>
					</View>
				</View>
